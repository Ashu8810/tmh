...
export default function FutureGoalsSection() {
  const goals = [
    { title: "Compete internationally every year", icon: <Globe className="w-4 h-4" /> },
    { title: "Develop advanced technologies", icon: <Cpu className="w-4 h-4" /> },
    { title: "Build sustainable & electric vehicles", icon: <Zap className="w-4 h-4" /> },
    { title: "Inspire and mentor future engineers", icon: <GraduationCap className="w-4 h-4" /> }
  ];
  ...
  return (
    <div className="w-full relative">
      ...
      <div className="mb-6 relative z-10">
        <span className="font-mono text-[#D71920] text-xl block mb-2 font-bold tracking-widest">08</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
        {goals.map((goal, index) => (
          <div key={index} className="p-4 bg-[#0a0a0a] rounded border border-white/5 relative group hover:border-white/20 transition-colors">
