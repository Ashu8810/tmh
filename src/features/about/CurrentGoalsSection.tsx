...
export default function CurrentGoalsSection() {
  const goals = [
    { title: "Improve vehicle performance", icon: <Settings className="w-4 h-4" /> },
    { title: "Strengthen team & departments", icon: <Users className="w-4 h-4" /> },
    { title: "Achieve better competition results", icon: <Trophy className="w-4 h-4" /> },
    { title: "Enhance infrastructure & resources", icon: <Wrench className="w-4 h-4" /> }
  ];

  return (
    <div className="w-full">
      <div className="mb-6">
        <span className="font-mono text-[#D71920] text-xl block mb-2 font-bold tracking-widest">07</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {goals.map((goal, index) => (
          <div key={index} className="p-4 bg-[#0a0a0a] rounded border border-white/5 relative group hover:border-[#D71920]/40 transition-colors">
